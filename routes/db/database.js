...
// Création du compte admin par défaut, uniquement au premier démarrage
if (!dbExistaitDeja) {
  ...
  console.log('✅ Base de données créée. Compte admin initialisé (identifiant: admin).');
  if (!process.env.BD_ADMIN_INIT_PASS) {
    console.log('⚠️  Mot de passe temporaire par défaut utilisé : ChangeMoiMaintenant!2024');
    console.log('⚠️  Changez-le immédiatement via /?admin=true → 🔒 Sécurité');
  }
}
